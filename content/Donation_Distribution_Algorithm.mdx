...
```plaintext
for each leaf issue v in L:
    compute P_v(t) = sum(δ_{j,v}(t))
    let C_v(t) = {c_i | v in S(c_i)}
    for each c_i in C_v(t):
        compute R_{i,v}(t) = {d_j in J(c_i) | r_{i,j,v}(t) exists}
        compute \bar{r}_{i,v}(t) = avg(r_{i,j,v}(t)) if R_{i,v}(t) ≠ ∅, else 0
    compute R_v(t) = sum(\bar{r}_{i,v}(t))
    for each c_i in C_v(t):
        p_{i,v}(t) = \bar{r}_{i,v}(t) / R_v(t) if R_v(t) > 0, else 0
        A_{i,v}(t) = p_{i,v}(t) * P_v(t)
    for each c_i in C_v(t):
        for each d_j in D with δ_{j,v}(t) > 0:
            a_{j,i,v}(t) = δ_{j,v}(t) * p_{i,v}(t)
            if D_{j,i}(t-1) + a_{j,i,v}(t) > K_{j,i}:
                e_{j,i,v}(t) = D_{j,i}(t-1) + a_{j,i,v}(t) - K_{j,i}
                a'_{j,i,v}(t) = a_{j,i,v}(t) - e_{j,i,v}(t)
                if d_j chooses refund:
                    return e_{j,i,v}(t) to d_j
                else:
                    δ_{j,parent(v)}(t) += e_{j,i,v}(t)
        A'_{i,v}(t) = sum(a'_{j,i,v}(t))
    update D_{j,i}(t) for all d_j, c_i
for each non-leaf issue v with updated δ_{j,v}(t):
    propagate funds upward or to leaf descendants
```
